...
RefModel = Backbone.Model.extend({
	...
	loadJSON: function( data ){
		...
		if( this.refModel ){
			...
			this[ fieldName ] = generator.call(this, data[ fieldName ] );
	...
	makeJSON: function(){
		...
		if( this.refCollection ){
			var coln = this[ this.refCollection.field ];
			if( coln )
				if( coln.makeJSON )
					data[ this.refCollection.field ] = coln.makeJSON();
				else
					data[ this.refCollection.field ] = coln.toJSON();
		}
		return data;
	}
});
